...
import { AdoptStage, diagnoses, stages } from '@/services/adoptDiagnostic';

type EngineState = 'diagnose' | 'analyzing' | 'results';
type WaveState = 'idle' | 'listening' | 'submitting' | 'analyzing' | 'transitioning' | 'results';

const suggestions = ['Users are trying once but not returning', 'High awareness but low activation', 'Feature discovery is under 5%'];
...
function App() {
  ...
  const startDiagnosis = () => {
    if (!input.trim() || engineState !== 'diagnose') return;
